...
# Format: API_KEY_<ROLE>=<key_value>
API_KEYS = {
...
def get_role_from_api_key(api_key: str) -> Optional[str]:
    """
    Determine the role associated with the provided API key.
    
    Args:
        api_key: The API key to validate
        
    Returns:
        Role name if valid, None otherwise
    """
...
def verify_api_key(api_key: Optional[str] = Security(api_key_header)) -> str:
    """
    Verify the API key and return the associated role.
...
def require_permission(required_permission: str):
    """
    Dependency to check if the authenticated user has the required permission.
    
    Args:
        required_permission: Permission required for the endpoint
        
    Returns:
        Dependency function
    """
    def permission_checker(role: str = Security(verify_api_key)) -> str:
        permissions = ROLE_PERMISSIONS.get(role, [])
        if required_permission not in permissions:
            logger.warning(
                f"Permission denied: role '{role}' lacks '{required_permission}' permission"
            )
            raise HTTPException(
